...
if [ -z $B ]; then
  ...
  # Clear the color after that
  clear='\033[0m'
  printf "${green}Replication setup started${clear}! \n"
fi
banner "The server's IP and slave ssh port"
...
timeout  60s yum install http://dev.mysql.com/get/mysql80-community-release-el8-5.noarch.rpm -y >> /tmp/replication_setup.log 2>&1
if [[ $(echo $?) -gt 0 ]]; then
...
timeout 60s dnf module disable mysql:8.0 -y >> /tmp/replication_setup.log 2>&1
timeout 60s dnf config-manager --enable mysql80-community >> /tmp/replication_setup.log 2>&1
timeout 60s dnf install mysql-community-server -y >> /tmp/replication_setup.log 2>&1
if [[ $(echo $?) -gt 0 ]]; then
...
mysql -e "create user 'replicant'@'$slaveip' IDENTIFIED BY '#Q5fdjYUF1XFSi9H9';GRANT REPLICATION SLAVE ON *.* to 'replicant'@'$slaveip';flush privileges" >> /tmp/replication_setup.log 2>&1
fi
firewall-cmd --add-rich-rule='rule family=ipv4 source address="'$slaveip'" port port="3306" protocol=tcp accept' --permanent >> /tmp/replication_setup.log 2>&1
firewall-cmd --reload >> /tmp/replication_setup.log 2>&1
echo " Taking mysqldump: "
ssh -p $sshport -o StrictHostKeyChecking=no root@$slaveip "mv /tmp/all.sql.gz /tmp/all.sql.gz_`date +"%d-%m-%Y-%s"`" >> /tmp/replication_setup.log 2>&1
...
scp -P $sshport -o StrictHostKeyChecking=no -pr /tmp/all.sql.gz root@$slaveip:/tmp/ >> /tmp/replication_setup.log 2>&1
if [[ $(echo $?) -gt 0 ]]; then
...
#timeout 3s ssh -o StrictHostKeyChecking=no root@$slaveip "uptime" >> /tmp/replication_setup.log 2>&1timeout 10s ssh -p $sshport -oBatchMode=yes -o StrictHostKeyChecking=no root@$slaveip echo"  > /dev/null 2>&1
if [[ $(echo $?) -gt 0 ]]; then
   echo "Root passwordless ssh connectivity to slave server failed"
   exit
else
echo "===================================slave server logs started=============" >> /tmp/replication_setup.log
#ssh -p $sshport -o StrictHostKeyChecking=no root@$slaveip  /tmp/replication_slave.sh
ssh -p $sshport -o StrictHostKeyChecking=no root@$slaveip  "bash <(curl -sL https://raw.githubusercontent.com/vimoshmohan/mysqllab/main/slave_scripts/replication_slave_el8.sh)"
if [[ $(echo $?) -gt 0 ]]; then
